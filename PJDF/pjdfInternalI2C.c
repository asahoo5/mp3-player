...
static PjdfErrCode ReadI2C(DriverInternal *pDriver, void* pBuffer, INT32U* pCount)
{
  ...
  // if Address is null, hang here - later an error code to be added as response
  if (pContext->i2CDevAddr == NULL) while(1);
  
  INT32U lCount = *pCount, numBytes = 0;
  
  OS_CPU_SR cpu_sr = 0;
  OS_ENTER_CRITICAL();
  
  // Send Start Condition along with the address (data to be read from) -- This step is valid for address-data communication model
  I2C_start(pContext->i2cMemMap, (INT8U)pContext->i2CDevAddr, LL_I2C_GENERATE_START_WRITE, LL_I2C_MODE_AUTOEND, 1u);
  I2C_write(pContext->i2cMemMap, ((INT8U*)pBuffer)[0]);
  // Wait for stop condition flag to be set
  BspI2c_WaitWithTimeoutReset(LL_I2C_IsActiveFlag_STOP, 1);
  LL_I2C_ClearFlag_STOP(pContext->i2cMemMap);
  
  // Send a repeated start condition & slave address along with corresponding mode with respect to data size.
  if(lCount <= MAX_NBYTE_SIZE)
  {
      numBytes = lCount;
      I2C_start(pContext->i2cMemMap, (INT8U)pContext->i2CDevAddr, LL_I2C_GENERATE_START_READ, LL_I2C_MODE_AUTOEND, (INT8U)numBytes);
  }
  else
  {
      numBytes = MAX_NBYTE_SIZE;
      I2C_start(pContext->i2cMemMap, (INT8U)pContext->i2CDevAddr, LL_I2C_GENERATE_START_READ, LL_I2C_MODE_RELOAD, (INT8U)numBytes);
  }
  
  INT32U numData = lCount; 
  // Start Sending Data
  for(INT32U i = 0; i < numData; ++i)
  {
     if( lCount == 1)
     ...
     if(lCount != 0 && numBytes == 0)
     {
         
         if(lCount <= MAX_NBYTE_SIZE)
         {
             numBytes = lCount;
             I2C_start(pContext->i2cMemMap, ((INT8U*)pBuffer)[0], LL_I2C_GENERATE_NOSTARTSTOP, LL_I2C_MODE_AUTOEND, (INT8U)numBytes);
         }
         else
         {
             numBytes = MAX_NBYTE_SIZE;
             I2C_start(pContext->i2cMemMap, ((INT8U*)pBuffer)[0], LL_I2C_GENERATE_NOSTARTSTOP, LL_I2C_MODE_RELOAD, (INT8U)numBytes);
         }  
     }
  }
  
  // With Autoend mode, stop condition is already created, No need to call I2C_stop()
  
  // Clear Stop Flag for next transaction
  LL_I2C_ClearFlag_STOP(pContext->i2cMemMap);
...
static PjdfErrCode WriteI2C(DriverInternal *pDriver, void* pBuffer, INT32U* pCount)
{
  ...
  INT32U lCount = (*pCount) + 1, numBytes = 0;
  
  OS_CPU_SR cpu_sr = 0;
  OS_ENTER_CRITICAL();
  
  // Send a start condition & slave address along with corresponding mode with respect to data size.
  if(lCount <= MAX_NBYTE_SIZE)
  {
      numBytes = lCount;
      I2C_start(pContext->i2cMemMap, (INT8U)pContext->i2CDevAddr, LL_I2C_GENERATE_START_WRITE, LL_I2C_MODE_AUTOEND, (INT8U)numBytes);
  }
  else
  {
      numBytes = MAX_NBYTE_SIZE;
      I2C_start(pContext->i2cMemMap, (INT8U)pContext->i2CDevAddr, LL_I2C_GENERATE_START_WRITE, LL_I2C_MODE_RELOAD, (INT8U)numBytes);
  }
